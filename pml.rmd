...
```{r model1, echo=TRUE, cache=FALSE}
summary(model1)
```

In-sample accuracy is 98% with sensitivity of the prediction > 95% and specificity > 99%.
```{r in_sample, echo=TRUE, cache=TRUE}
inPred <- predict(model1)
confusionMatrix(training$classe, inPred)
```

After cross validation, out of sample accuracy is 96% with sensitivity of the prediction > 92% and specificity > 98%.
